...
public class WebSearchDocumentRetriever implements DocumentRetriever {
    ...
    private static String TAVILY_BASE_URL;
    private static final int DEFAULT_RESULT_LIMIT = 5;
    private final int resultLimit;
    private final RestClient restClient;

    public WebSearchDocumentRetriever(RestClient.Builder clientBuilder, int resultLimit, TavilyProperties properties) {
        TAVILY_API_KEY = properties.apiKey();
        TAVILY_BASE_URL = properties.baseUrl();
        Assert.notNull(clientBuilder, "clientBuilder cannot be null");
        Assert.hasText(TAVILY_API_KEY, "Environment variable " + TAVILY_API_KEY + " must be set");
    ...
    public static class Builder {
        ...
        public WebSearchDocumentRetriever build(TavilyProperties properties) {
            return new WebSearchDocumentRetriever(clientBuilder, resultLimit, properties);
        }
    }
}
